async function drawScatter() {
  ...
  console.log(beeAccessor(bee_data[2]))
  console.log(d3.extent(bee_data, beeAccessor))
  console.log("size")
  ...
  const beeSizeScale = await d3.scaleLinear()
    .domain(d3.extent(bee_data, beeAccessor))
    .range([0, 8])

    console.log("minmax")
    console.log((beeAccessor(d3.min(bee_data))))
    console.log((beeAccessor(d3.max(bee_data))))
    console.log((d3.extent(bee_data, beeAccessor)[0]))
  ...
  const bees = bounds.selectAll("circle")
  .data(bee_data)
  .enter().append("circle")
    .attr("cx", d => xScale(xAccessor(d)))
    .attr("cy", d => yScale(beeAccessor(d)))
    .attr("r", d => beeSizeScale(beeAccessor(d)))
    .attr("fill", "black")
    .attr("tabindex", "0")
